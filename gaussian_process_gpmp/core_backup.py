...
class Model:
    ...
    def __init__(self, mean, covariance, meanparam=None, covparam=None, meantype="linear_predictor"):
        """
        Parameters
        ----------
        mean : callable
    ...
    def negative_log_likelihood_zero_mean(self, covparam, xi, zi):
        ...
        try:
            ...
            return gnp.tensor(float("inf"), requires_grad=True)
    ...
    def negative_log_restricted_likelihood(self, covparam, xi, zi):
        ...
        Pshape        = P.shape
        n, q          = Pshape
        [Q, R]        = gnp.qr(P, "complete") # Compute a matrix of contrasts
        W             = Q[:, q:n]
        Wzi           = gnp.matmul(W.T, zi) # Contrasts (n-q) x 1
        G             = gnp.matmul(W.T, gnp.matmul(K, W)) # Compute G = W' * (K * W), the covariance matrix of contrasts
        try:
            WKWinv_Wzi, C = gnp.cholesky_solve(G, Wzi) # Cholesky factorization: G = U' * U, with upper-triangular U and Compute G^(-1) * (W' zi)
        except:
            return gnp.tensor(float("inf"), requires_grad=True)
        norm2         = gnp.einsum("i..., i...", Wzi, WKWinv_Wzi) # Compute norm2 = (W' zi)' * G^(-1) * (W' zi)
        ldetWKW       = 2.0 * gnp.sum(gnp.log(gnp.diag(C))) # Compute log(det(G)) using the Cholesky factorization
        L             = 0.5 * ((n - q) * gnp.log(2.0 * gnp.pi) + ldetWKW + norm2)
        return L.reshape(())

    def profiled_negative_log_likelihood_zero_mean(self, covparam, xi, zi):
        K          = self.covariance(xi, xi, torch.cat((torch.tensor([0]), covparam)))
        n          = K.shape[0]
        try:
            Kinv_zi, C = gnp.cholesky_solve(K, zi)
        except:
    ...
    def profiled_pva_negative_log_likelihood_zero_mean(self, covparam, xi, zi):
        K              = self.covariance(xi, xi, torch.cat((torch.tensor([0]), covparam)))
        n              = K.shape[0]
        try:
            Kinv_zi, C     = gnp.cholesky_solve(K, zi)
    ...
    def mean_square_error(self, covparam, xi, zi):
        K              = self.covariance(xi, xi, torch.cat((torch.tensor([0]), covparam)))
        K_inv          = gnp.inv(K)
        K_inv_diag_inv = gnp.inv(gnp.diag(gnp.diag(K_inv)))
    ...
    def compute_log_sigma2_pmle(self, covparam, xi, zi):
        K          = self.covariance(xi, xi, torch.from_numpy(covparam))
        n          = K.shape[0]
        Kinv_zi, C = gnp.cholesky_solve(K, zi)
        norm2      = gnp.einsum("i..., i...", zi, Kinv_zi)
        log_sigma2 =  gnp.log(norm2 / n)
        return log_sigma2.reshape(())

    def compute_log_sigma2_pva(self, covparam, xi, zi):
        K              = self.covariance(xi, xi, torch.from_numpy(covparam))
        n              = K.shape[0]
        K_inv          = gnp.inv(K)
        K_inv_diag_inv = gnp.inv(gnp.diag(gnp.diag(K_inv)))
        pva_matrix     = torch.mm(K_inv, torch.mm(K_inv_diag_inv, K_inv))
        pva_norm2      = gnp.einsum("i..., i...", zi, torch.mv(pva_matrix, zi))
        log_sigma2     = gnp.log(pva_norm2 / n)
        return log_sigma2.reshape(())
